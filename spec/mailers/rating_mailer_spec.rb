...
describe RatingMailer do
  describe 'rating_notification' do
    before do
      ...
      approved: true, deactivated: false)
    	@mail = RatingMailer.rating_notification(@users, @book)
    end
 
    it 'renders the subject' do
      expect(@mail.subject).to eql("American Gods has been rated on Bookbee!")
    end
    ...
    it 'renders the sender email' do
      expect(@mail.from).to eql(["ratings@example.com"])
    end
  ...
  describe 'creator_notification' do
    before do
      ...
      isbn: "9783161484100", user_id: 101, cover: "image.jpg", 
      approved: true, deactivated: false)
    	@new_mail = RatingMailer.creator_notification(@new_user, @book)
    end
 
    it 'renders the subject' do
      expect(@new_mail.subject).to eql("Your book American Gods has been rated on Bookbee!")
    end
    ...
    it 'renders the sender email' do
      expect(@new_mail.from).to eql(["ratings@example.com"])
    end
